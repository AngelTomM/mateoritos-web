...
// Simple i18n for texts generated by the sketch
function getLang() {
  return localStorage.getItem('mateoritos_lang') || (navigator.language && navigator.language.startsWith('en') ? 'en' : 'es');
}

const I18N = {
  es: {
    current_values: 'Valores actuales:',
    a_label: 'a (semieje mayor)',
    e_label: 'e (excentricidad)',
    i_label: 'i (inclinación)',
    perigee: 'Perigeo',
    moid_label: 'MOID',
    km: 'km',
    danger: '⚠️ ¡PELIGRO! amenaza de asteroide ⚠️'
  },
  en: {
    current_values: 'Current values:',
    a_label: 'a (semi-major axis)',
    e_label: 'e (eccentricity)',
    i_label: 'i (inclination)',
    perigee: 'Perigee',
    moid_label: 'MOID',
    km: 'km',
...
// respond to language changes from other tabs
window.addEventListener('storage', (ev) => {
  if (ev.key === 'mateoritos_lang') {
    // nothing to do immediately; draw() reads language each frame and will display updated text
  }
});
...
function draw() {
  ...
  infoDiv.html(`
    <b>${t.current_values}</b><br>
    ${t.a_label}: ${a.toFixed(6)} UA<br>
    ${t.e_label}: ${e.toFixed(3)}<br>
    ${t.i_label}: ${i.toFixed(2)}°<br>
    ${t.perigee}: ${perigeoAU.toFixed(6)} UA / ${(perigeoAU * AU_IN_KM).toLocaleString(lang || undefined, {maximumFractionDigits:0})} ${t.km}<br>
    ${t.moid_label}: ${moid.toFixed(6)} UA / ${(moid * AU_IN_KM).toLocaleString(lang || undefined, {maximumFractionDigits:0})} ${t.km}
  `);

  peligroDiv.html(peligro ? t.danger : '');
